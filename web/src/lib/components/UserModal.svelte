<script lang="ts">
  ...
  export let onsuccess: ((data: User) => void) | null = null;
...
<Modal bind:isOpen title={modalTitle} size="md" onclose={handleClose}>
    ...
    <div class="space-y-1.5">
      ...
      >
        ...
        {#if isEditing}
          ...
          >
        {:else}
          <span class="text-destructive text-xs font-normal"
            >Required (min 8 chars)</span
          >
        {/if}
      </label>
      <Input
  ...
  <svelte:fragment slot="footer">
    ...
    <Button onclick={handleSubmit} {loading} disabled={!email.trim()}>
