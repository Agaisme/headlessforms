<script lang="ts">
  ...
  export let form: Form | null = null;
  export let onsuccess: ((data: Form) => void) | null = null;
...
<Modal bind:isOpen title={modalTitle} size="lg" onclose={handleClose}>
  ...
  >
    {#if error}
      <div
        class="bg-destructive/10 text-destructive p-4 rounded-lg flex items-center gap-3 text-sm"
      >
        <svg
    ...
    <!-- Access Mode -->
    <div class="space-y-1.5" role="group" aria-labelledby="access-mode-label">
      <span id="access-mode-label" class="text-sm font-medium">Access Mode</span
      >
      <div class="grid grid-cols-3 gap-2">
        <button
          type="button"
          class="p-3 rounded-lg border text-left transition-all {accessMode ===
          'public'
    ...
    {#if accessMode === "with_key"}
      <div class="space-y-1.5 p-4 rounded-lg bg-muted/30 border border-border">
        ...
        />
        <p class="text-xs text-muted-foreground">
          Add this as a hidden field: <code class="bg-muted px-1 rounded"
            >&lt;input type="hidden" name="_submission_key" value="{submissionKey ||
              "YOUR_KEY"}"&gt;</code
          >
        </p>
      </div>
    ...
    <div class="space-y-1.5">
      ...
      />
      <p class="text-xs text-muted-foreground">
        Comma-separated list of emails to notify on new submissions
      </p>
    </div>
    ...
    <div class="border border-border rounded-lg overflow-hidden">
      ...
      >
        ...
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="h-4 w-4 text-muted-foreground transition-transform {showAdvanced
            ? 'rotate-180'
            : ''}"
          viewBox="0 0 20 20"
          fill="currentColor"
      ...
      {#if showAdvanced}
        <div class="px-4 pb-4 space-y-4 border-t border-border">
          <div class="pt-4 space-y-1.5">
            ...
            />
            <p class="text-xs text-muted-foreground">
              We'll POST submission data to this URL
            </p>
          </div>
