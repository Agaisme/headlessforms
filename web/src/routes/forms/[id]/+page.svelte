<script lang="ts">
	...
	async function loadData() {
		...
		try {
			...
			const subRes = await fetch(`/api/v1/forms/${formId}/submissions`, { headers });
			const subJson = await subRes.json();
			if (subJson.status === 'success') {
				// Handle both paginated and non-paginated response
				const submissionList = subJson.data?.submissions || subJson.data || [];
				submissions = submissionList.sort((a: any, b: any) => 
	...
	function getSubmissionData(sub: any) {
		if (!sub) return {};
	...
	function getPreviewFields(sub: any) {
		...
		const name = data.name || data.firstName || data.Name || data.first_name || '';
		const subject = data.subject || data.message?.substring(0, 50) || data.comments?.substring(0, 50) || 'Form Submission';
...
<div class="space-y-6">
	...
	<div class="flex items-center justify-between">
		...
		<div class="flex items-center gap-2">
			<Button variant="ghost" size="sm" class="gap-2" onclick={() => showTestForm = !showTestForm}>
				<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
			...
			<Button variant="outline" size="sm" class="gap-2" href="/forms/{formId}/edit">
				<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
			...
			<Card>
				<CardContent class="p-5">
					<div class="flex items-center justify-between mb-4">
						...
						<Button variant="ghost" size="icon" onclick={() => showTestForm = false} aria-label="Close settings">
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<div>
							...
							<div class="flex items-center gap-2">
								<code class="flex-1 bg-muted px-3 py-2 rounded text-xs font-mono truncate">POST {getEndpointUrl()}</code>
							...
							<p class="text-xs text-muted-foreground mt-2">Status: <Badge variant="success">{form?.status || 'active'}</Badge></p>
						</div>
						...
						<div>
							...
							{#if submitSuccess}
								<div class="bg-green-500/10 text-green-600 p-3 rounded-lg text-sm">✅ Sent!</div>
							{:else}
								<form onsubmit={(e) => { e.preventDefault(); handleTestSubmit(); }} class="flex gap-2">
									<Input type="email" class="flex-1" placeholder="test@email.com" bind:value={testEmail} required />
		...
		<Card class="overflow-hidden">
			...
			<div class="flex items-center justify-between px-5 py-4 border-b border-border">
				...
				<div class="flex items-center gap-2">
					...
					{/if}
					<span class="text-sm text-muted-foreground">{submissions.length} total</span>
				</div>
			</div>
			...
			{#if submissions.length === 0}
				<div class="text-center py-16">
					<div class="w-20 h-20 rounded-full bg-muted flex items-center justify-center mx-auto mb-4">
						<svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-muted-foreground/50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
						</svg>
					</div>
					<h3 class="text-lg font-semibold mb-2">Inbox is empty</h3>
					<p class="text-muted-foreground mb-4">Submissions from your landing page will appear here.</p>
					<Button variant="outline" size="sm" onclick={() => showTestForm = true}>Send a Test Submission</Button>
				</div>
			{:else}
				<div class="flex min-h-[500px]">
					<!-- Email List (Left Panel) -->
					<div class="w-80 border-r border-border overflow-y-auto max-h-[600px]">
						{#each submissions as sub}
							...
							<button 
								class="w-full text-left px-4 py-3 border-b border-border/50 hover:bg-muted/50 transition-colors
								       {selectedSubmission?.id === sub.id ? 'bg-primary/5 border-l-2 border-l-primary' : ''}
								...
								<div class="flex items-start justify-between gap-2">
									<div class="flex-1 min-w-0">
										<div class="flex items-center gap-2">
											...
											{/if}
											<p class="font-semibold text-sm truncate {sub.status === 'unread' ? '' : 'font-normal'}">
												{preview.name || preview.email || 'Anonymous'}
											</p>
										</div>
										{#if preview.email && preview.name}
											<p class="text-xs text-muted-foreground truncate pl-4">{preview.email}</p>
										{/if}
					...
					<div class="flex-1 overflow-y-auto max-h-[600px]">
						{#if selectedSubmission}
							...
							<!-- Email Header -->
							<div class="px-6 py-4 border-b border-border bg-card sticky top-0">
								<div class="flex items-start justify-between">
									<div>
										...
										{#if preview.email}
											<p class="text-sm text-muted-foreground">{preview.email}</p>
										{/if}
									...
									<div class="flex items-center gap-1">
										{#if selectedSubmission.status === 'read'}
											<Button variant="ghost" size="icon" onclick={() => markAsUnread(selectedSubmission.id)} title="Mark as unread">
												<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
													<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
												</svg>
											</Button>
										{:else}
											<Button variant="ghost" size="icon" onclick={() => markAsRead(selectedSubmission.id)} title="Mark as read">
												<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
													<path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
												</svg>
											</Button>
										{/if}
										<Button variant="ghost" size="icon" class="text-destructive" onclick={() => deleteSubmission(selectedSubmission.id)} title="Delete">
											<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
							...
							<div class="p-6">
										...
										<div class="border-b border-border/50 pb-3">
											...
											<div class="text-foreground">
												...
												{:else if typeof value === 'object'}
													<pre class="text-xs bg-muted p-2 rounded mt-1">{JSON.stringify(value, null, 2)}</pre>
												{:else}
													{value}
												{/if}
											</div>
								...
								<div class="mt-6 pt-4 border-t border-border flex gap-2">
									<Button variant="ghost" size="sm" class="gap-2" onclick={() => copyToClipboard(JSON.stringify(data, null, 2))}>
										<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
											<path d="M8 3a1 1 0 011-1h2a1 1 0 110 2H9a1 1 0 01-1-1z" />
											<path d="M6 3a2 2 0 00-2 2v11a2 2 0 002 2h8a2 2 0 002-2V5a2 2 0 00-2-2 3 3 0 01-3 3H9a3 3 0 01-3-3z" />
										</svg>
