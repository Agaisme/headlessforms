  ...
  {:else}
    ...
    <Card>
      ...
      <CardContent class="space-y-4">
        <div>
          <label for="formName" class="text-sm font-medium mb-1.5 block"
            >Form Name</label
          >
          <Input id="formName" bind:value={name} placeholder="Contact Form" />
        </div>
        ...
        <div>
          ...
          <div class="flex gap-2">
            <code
              class="flex-1 bg-muted px-3 py-2 rounded text-sm font-mono truncate"
              >{getEndpointUrl()}</code
            >
            <Button
              variant="ghost"
        ...
        <div>
          <label for="redirectUrl" class="text-sm font-medium mb-1.5 block"
            >Redirect URL (after submission)</label
          >
          <Input
            id="redirectUrl"
        ...
        <div>
          <label for="notifyEmails" class="text-sm font-medium mb-1.5 block"
            >Notification Emails (comma separated)</label
          >
          <Input
            id="notifyEmails"
    ...
    <Card>
      <CardHeader class="pb-3">
        ...
        <p class="text-sm text-muted-foreground">
          Control who can submit to this form
        </p>
      </CardHeader>
      <CardContent class="space-y-4">
        ...
        {#if accessMode === "with_key"}
          <div class="p-4 bg-muted/50 rounded-lg space-y-3">
            <div class="flex items-center justify-between">
              <label for="submissionKey" class="text-sm font-medium"
                >Submission Key</label
              >
              <Button variant="ghost" size="sm" onclick={generateSubmissionKey}
                >Generate New</Button
              >
            </div>
            <div class="flex gap-2">
              ...
              <Button
                variant="ghost"
                size="icon"
                onclick={() => copyToClipboard(submissionKey)}
                title="Copy Key"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
            ...
            <p class="text-xs text-muted-foreground">
              Add this as a hidden field in your form: <code
                class="bg-muted px-1 rounded">_submission_key</code
              >
            </p>
          </div>
        {/if}
    ...
    <Card>
      <CardHeader class="pb-3">
        ...
        <p class="text-sm text-muted-foreground">
          Send submissions to an external URL
        </p>
      </CardHeader>
      <CardContent class="space-y-4">
        <div>
          <label for="webhookUrl" class="text-sm font-medium mb-1.5 block"
            >Webhook URL</label
          >
          <Input
            id="webhookUrl"
        ...
        <div>
          <label for="webhookSecret" class="text-sm font-medium mb-1.5 block"
            >Webhook Secret (for HMAC signature)</label
          >
          <Input
            id="webhookSecret"
