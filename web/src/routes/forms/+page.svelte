<script lang="ts">
  ...
  async function selectSubmission(sub: Submission) {
    ...
    if (sub.status === "unread") {
      try {
        ...
        console.warn("Failed to mark submission as read:", e);
      }
    }
  }
  ...
  async function handleFormSuccess(_form: Form) {
    await loadForms();
  }
...
<div class="flex gap-4 h-[calc(100vh-8rem)] lg:h-[calc(100vh-8rem)]">
  <!-- Panel 1: Form List -->
  <div
    class="w-full lg:w-72 flex-shrink-0 flex flex-col bg-card border border-border rounded-xl overflow-hidden {mobileView !==
    'forms'
      ? 'hidden lg:flex'
      : 'flex'}"
  >
    <!-- Desktop Header -->
    <div
      class="hidden lg:flex p-4 border-b border-border items-center justify-between"
    >
    ...
    <div class="flex-1 overflow-y-auto">
      {#if loading}
        <div class="p-4 space-y-3">
          {#each [1, 2, 3, 4, 5] as _}
            <div
              class="flex items-center gap-3 p-3 rounded-lg bg-muted/30 animate-pulse"
            >
      ...
      {:else}
        {#each forms as form}
          <button
            class="w-full px-4 py-3 text-left border-b border-border hover:bg-muted/50 transition-all duration-200 {selectedForm?.public_id ===
            form.public_id
          ...
          >
            <div class="flex items-center gap-3">
                ...
                <div class="flex items-center justify-between">
                  ...
                  {#if (form.submission_count ?? 0) > 0}
                    <Badge variant="secondary" class="text-xs ml-2"
                      >{form.submission_count}</Badge
                    >
                  {/if}
                </div>
                <p class="text-xs text-muted-foreground mt-0.5">
                  {form.access_mode === "private"
                    ? "🔒"
                    : form.access_mode === "with_key"
                      ? "🔑"
                      : "🌐"}
              ...
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="h-4 w-4 text-muted-foreground lg:hidden"
                viewBox="0 0 20 20"
                fill="currentColor"
              >
                <path
  ...
  <div
    class="w-full lg:w-80 flex-shrink-0 flex flex-col bg-card border border-border rounded-xl overflow-hidden {mobileView !==
    'inbox'
      ? 'hidden lg:flex'
      : 'flex'}"
  >
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex gap-1">
            <Button
              ...
              size="sm"
              onclick={() =>
                selectedForm && loadSubmissions(selectedForm.public_id)}
              title="Refresh"
              class="h-8 w-8 p-0"
            ...
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="h-4 w-4 {loadingSubmissions ? 'animate-spin' : ''}"
                viewBox="0 0 20 20"
                fill="currentColor"
              >
                <path
            ...
            <Button
              variant="ghost"
              size="sm"
              href="/forms/{selectedForm.public_id}"
              title="Full view"
              class="h-8 w-8 p-0"
            >
              <svg
      ...
      <div class="flex-1 overflow-y-auto">
        {#if loadingSubmissions}
          <div class="p-4 space-y-3">
            {#each [1, 2, 3, 4] as _}
              <div
                class="flex items-start gap-3 p-3 rounded-lg bg-muted/30 animate-pulse"
              >
        ...
        {:else if submissions.length === 0}
          <div
            class="flex-1 flex flex-col items-center justify-center p-8 text-muted-foreground"
          >
            <div
        ...
        {:else}
          {#each submissions as sub}
            <button
              class="w-full px-4 py-3 text-left border-b border-border hover:bg-muted/50 transition-all duration-200 {selectedSubmission?.id ===
              sub.id
                ? 'bg-primary/5'
                : ''}"
            ...
            >
              <div class="flex items-start justify-between gap-3">
                <div class="flex items-start gap-3 flex-1 min-w-0">
                  {#if sub.status === "unread"}
                    <span
                      class="w-2 h-2 rounded-full bg-primary flex-shrink-0 mt-2 animate-pulse"
                    ></span>
                  {:else}
                    <span
                      class="w-2 h-2 rounded-full bg-transparent flex-shrink-0 mt-2"
                    ></span>
                  {/if}
                  <div class="flex-1 min-w-0">
                    <span
                      class="font-medium text-sm truncate block {sub.status ===
                      'unread'
                        ? ''
                        : 'text-muted-foreground'}"
                      >{getSubmissionPreview(sub)}</span
                    >
                    <p class="text-xs text-muted-foreground truncate mt-0.5">
                      {getSubmissionSubject(sub)}
                    </p>
                  </div>
                </div>
    ...
    {:else}
      <div
        class="flex-1 flex items-center justify-center text-muted-foreground p-8"
      >
  ...
  <div
    class="w-full lg:flex-1 flex flex-col bg-card border border-border rounded-xl overflow-hidden {mobileView !==
    'detail'
      ? 'hidden lg:flex'
      : 'flex'}"
  >
      ...
      <div class="p-4 border-b border-border flex items-center justify-between">
        <div class="min-w-0 flex-1">
          <h4 class="font-semibold truncate">
            {getSubmissionPreview(selectedSubmission)}
          </h4>
          <p class="text-xs text-muted-foreground">
            ...
            <div>
              ...
              <div class="bg-muted/30 rounded-xl p-4 overflow-x-auto">
                <pre class="text-xs">{JSON.stringify(
                    typeof selectedSubmission.meta === "string"
                      ? JSON.parse(selectedSubmission.meta)
                      : selectedSubmission.meta,
                    null,
                    2
                  )}</pre>
              </div>
