...
<div class="space-y-6">
	<Card>
		<!-- Header -->
		<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 border-b border-border">
			<div>
				<h2 class="text-lg font-semibold">User Management</h2>
				<p class="text-sm text-muted-foreground mt-0.5">Manage users and their access roles</p>
			</div>
		...
		{:else if users.length === 0}
			<div class="text-center py-20">
				...
				<p class="text-muted-foreground mb-6 max-w-sm mx-auto">Create your first user to start managing access.</p>
						...
						<TableRow class="group">
							...
							<TableCell class="text-right">
								<div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
									...
									{#if user.id !== $auth.user?.id}
										<Button variant="ghost" size="sm" onclick={() => handleDeleteUser(user.id, user.email)}>
...
<!-- User Modal -->
<UserModal bind:isOpen={showUserModal} user={editingUser} onsuccess={handleUserSuccess} />
