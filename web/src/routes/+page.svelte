  ...
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="text-muted-foreground">
        Here's an overview of your form submissions.
      </p>
    </div>
        ...
        <CardContent class="p-5">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p
                class="text-3xl font-bold mt-1 {(stats?.unread_submissions ??
                  0) > 0
                  ? 'text-yellow-500'
                  : ''}"
              >
                {stats?.unread_submissions ?? 0}
              </p>
            </div>
            <div
          ...
          {#if (stats?.unread_submissions ?? 0) > 0}
            <Button
              ...
              size="sm"
              href="/forms"
              class="text-xs p-0 h-auto mt-2">View inbox →</Button
            >
          {:else}
      ...
      <CardContent class="p-5">
        <div class="flex items-center justify-between mb-4">
          ...
          <Badge variant="outline"
            >{stats.submissions_this_week || 0} this week</Badge
          >
        </div>
        <div class="flex items-end justify-between gap-2 h-40">
          {#each stats.daily_submissions as day}
            {@const maxCount = Math.max(
              ...stats.daily_submissions.map((d: { count: number }) => d.count),
              1
            )}
            {@const heightPercent =
            ...
            <div class="flex-1 flex flex-col items-center gap-2">
              ...
              >
                <div
                  class="absolute -top-6 left-1/2 -translate-x-1/2 bg-foreground text-background text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap"
                >
    ...
    {#if $auth.user?.role === "super_admin"}
      ...
      >
        <a href="/settings" class="block">
          <CardContent class="p-5 flex items-center gap-4">
            <div
              class="w-12 h-12 rounded-xl bg-gray-500/10 flex items-center justify-center group-hover:bg-gray-500/20 transition-colors"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
