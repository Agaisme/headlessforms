<script lang="ts">
	...
	onMount(async () => {
		try {
			...
			setupRequired = json.data?.setup_required ?? false;
		} catch {
			setupRequired = false;
		}
		checkingSetup = false;
...
<svelte:head>
	<title>Register - HeadlessForms</title>
</svelte:head>
	...
	<div class="w-full max-w-md">
		...
		<Card class="shadow-xl">
			...
			<CardContent class="pt-4">
				{#if setupRequired}
					<div class="bg-primary/10 text-primary p-3 rounded-lg mb-4 text-sm flex items-center gap-2">
						<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 shrink-0" viewBox="0 0 20 20" fill="currentColor">
							<path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" />
						</svg>
						This will be the admin account with full access.
					</div>
				{/if}
				...
				<form onsubmit={(e) => { e.preventDefault(); handleSubmit(); }} class="space-y-4">
					...
					<Button type="submit" class="w-full" disabled={loading || !email || password.length < 8}>
						{#if loading}
